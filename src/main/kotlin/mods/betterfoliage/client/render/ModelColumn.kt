    ...
    listOf(
        ...
        verticalRectangle(
            ...
            .setAoShader(
                faceOrientedAuto(
                    overrideFace = SOUTH,
                    corner = cornerInterpolate(Axis.Y, chamferAffinity, Config.roundLogs.dimming)
                )
            )
            .setAoShader(
                faceOrientedAuto(
                    overrideFace = SOUTH,
                    corner = cornerInterpolate(Axis.Y, 0.5f, Config.roundLogs.dimming)
                ),
...
fun Model.columnLidSquare(transform: (Quad) -> Quad = { it }) {
    transform(
        horizontalRectangle(x1 = 0.0, x2 = 0.5, z1 = 0.0, z2 = 0.5, y = 0.5)
            .transformVI { vertex, idx ->
                vertex.copy(
                    uv = UV(vertex.xyz.x, vertex.xyz.z), aoShader = when (idx) {
                        0 -> FaceCenter(UP)
                        1 -> EdgeInterpolateFallback(UP, SOUTH, 0.0)
