...
inline fun <reified T> MutableList<T>.replace(transform: (T) -> T) =
    forEachIndexed { idx, value -> this[idx] = transform(value) }
...
class ThreadLocalDelegate<T>(init: () -> T) {
    var tlVal: ThreadLocal<T> = ThreadLocal.withInitial(init)
...
fun random(min: Double, max: Double) = random().let { min + (max - min) * it }

/**
 * Return this [Double] value if it lies between the two limits. If outside, return the
 * minimum/maximum value correspondingly.
 */
fun Double.minmax(minVal: Double, maxVal: Double) = this.coerceAtLeast(minVal).coerceAtMost(maxVal)

/**
 * Return this [Int] value if it lies between the two limits. If outside, return the
 * minimum/maximum value correspondingly.
 */
fun Int.minmax(minVal: Int, maxVal: Int) = this.coerceAtLeast(minVal).coerceAtMost(maxVal)
