...
fun cornerAoTri(func: (AoData, AoData) -> AoData) =
    { face: ForgeDirection, dir1: ForgeDirection, dir2: ForgeDirection ->
...
class CornerTri(
    val face: ForgeDirection, val dir1: ForgeDirection, val dir2: ForgeDirection,
...
class EdgeInterpolateFallback(
    ...
    val pos: Double,
    val fallbackDimming: Float = defaultEdgeDimming
) : Shader {
...
class CornerInterpolateDimming(
    val face1: ForgeDirection, val face2: ForgeDirection, val edgeDir: ForgeDirection,
    ...
    override fun shade(context: ShadingContext, vertex: RenderVertex) {
        ...
        if (!shading1.valid) {
            shading1 = shading2; weight1 *= dimming
        }
        if (!shading2.valid) {
            shading2 = shading1; weight2 *= dimming
        }
        vertex.shade(shading1, shading2, weight1, weight2)
    }

    override fun rotate(rot: Rotation) =
        CornerInterpolateDimming(
            face1.rotate(rot),
            face2.rotate(rot),
            edgeDir.rotate(rot),
            weight,
